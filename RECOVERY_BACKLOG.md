...
- decidir a ordem de migracao do consumidor:
...
- validar se vale corrigir permissao real de `.pytest-local/run` e `.pytest-tmp`; ruido no `git status` foi mitigado no slice 43 via higiene de workspace e `.gitignore`
